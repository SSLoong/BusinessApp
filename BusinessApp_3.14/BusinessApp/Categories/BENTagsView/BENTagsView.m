...
@synthesize tagColors = _tagColors;
...
- (void)baseInit
{
	...
	self.offColor = [UIColor colorWithRed:0.79 green:0.79 blue:0.79 alpha:0.3];
...
- (void)refresh
{
	...
	for (BENTag *tag in self.tags)
	{
		...
		{
			[self addConstraints:[NSLayoutConstraint constraintsWithVisualFormat:@"[previousTag]-10-[tag]"
																		 options:0
																		 metrics:nil
...
{
    if ([tagColors isEqualToArray:self.tagColors]) return;
    BOOL reuseTags = tagColors.count == self.tagColors.count;
