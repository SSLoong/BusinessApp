...
@interface SourceViewController ()<UIWebViewDelegate,UINavigationControllerDelegate, UIImagePickerControllerDelegate, UIAlertViewDelegate>
...
-(void)viewWillAppear:(BOOL)animated{
    ...
    _webView = [[UIWebView alloc] initWithFrame:CGRectMake(0, 20, self.view.bounds.size.width, self.view.bounds.size.height-20 - 49)];
...
- (void)loadWebView {
    if (self.url_type == 1) {
        NSString *urlString = [NSString stringWithFormat:@"%@/purchase/goods?shop_id=%@&goods_id=%@&dealer_id=%@",SITE_SERVER,Store_id,self.goods_id,self.dealer_id];
        
        NSURLRequest *req = [[NSURLRequest alloc] initWithURL:[NSURL URLWithString:urlString]];
        
        [_webView loadRequest:req];
    }else{
        NSString *urlString = [NSString stringWithFormat:@"%@/purchase?shop_id=%@&goods_id=&dealer_id=",SITE_SERVER,Store_id];
